...
export const ExampleValue = styled(FieldLabel)`
  ...
  ${({ theme }) => `
    ...
    padding: 2px ${theme.spacing.unit + 2}px;
...
export const ConstraintItem = styled(FieldLabel)`
  border-radius: 6px;
  ${({ theme }) => `
    background-color: ${transparentize(0.92, theme.colors.primary.light)};
    color: ${transparentize(0.1, theme.colors.primary.main)};

    margin: 0 ${theme.spacing.unit}px;
    padding: 2px ${theme.spacing.unit + 2}px;
    border: 1px solid ${transparentize(0.85, theme.colors.primary.main)};
}`};
  & + & {
    margin-left: 0;
  }
